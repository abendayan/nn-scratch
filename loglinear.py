...
def softmax(x):
    ...
    exp_x = np.exp(x - np.max(x)) # -max(x): to be sure the values are not too big (identity from ex2, q1)
...
def loss_and_gradients(x, y, params):
    ...
    W,b = params
    x_np = np.array(x)
    prediction = classifier_output(x_np, params)
    y_hot = np.zeros(W.shape[1])
    y_hot[y] = 1
    # it should be -sum y_hot*log(prediction), but y_hot is not 0 only at y
    loss = -np.log(prediction[y])
    gb = prediction - y_hot
    gW = np.outer(x_np, prediction - y_hot)
    return loss,[gW,gb]
...
if __name__ == '__main__':
    ...
    print "sanity checks for softmax passed"
